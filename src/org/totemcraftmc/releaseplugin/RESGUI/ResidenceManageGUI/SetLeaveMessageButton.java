...
public class SetLeaveMessageButton extends AbstractButton {
	...
	public InventoryInteractResponse onClick(InventoryAction action) {
		...
		if (action == InventoryAction.PICKUP_ALL) {
			AdvancedMessage m = new AdvancedMessage(ChatColor.GREEN + "请在聊天栏直接输入要设置的离开领地提示语，15秒内有效。  ");
			...
			ChatHookManager.registerHook(new ChatHook(getPlayer()) {
				...
				public void recieve(String str) {
					...
					getPlayer().sendMessage(ChatColor.GREEN+"已设置离开领地提示语为: "+ChatColor.YELLOW+str);
				}
			});
	...
	public void refresh() {
		...
		meta.setDisplayName(ChatColor.GREEN + "设置离开领地提示语");
		List<String> lore = new ArrayList<>();
		if (res.getLeaveMessage() != null) {
			lore.add(ChatColor.GOLD + "当前: " + res.getLeaveMessage());
			lore.add(ChatColor.RESET + "" + ChatColor.UNDERLINE + "按Q清除离开领地提示语");
		}
